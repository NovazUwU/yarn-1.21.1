...
extends EntityModel<E> {
    ...
    public void render(MatrixStack matrices, VertexConsumer vertices, int light, int overlay, int color) {
        if (this.child) {
            ...
            this.getHeadParts().forEach(arg3 -> arg3.render(matrices, vertices, light, overlay, color));
            matrices.pop();
            matrices.push();
            f = 1.0f / this.invertedChildBodyScale;
            matrices.scale(f, f, f);
            matrices.translate(0.0f, this.childBodyYOffset / 16.0f, 0.0f);
            this.getBodyParts().forEach(arg3 -> arg3.render(matrices, vertices, light, overlay, color));
            matrices.pop();
        } else {
            this.getHeadParts().forEach(arg3 -> arg3.render(matrices, vertices, light, overlay, color));
            this.getBodyParts().forEach(arg3 -> arg3.render(matrices, vertices, light, overlay, color));
